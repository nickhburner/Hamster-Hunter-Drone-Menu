...
def _add_info_icon(parent_frame, tooltip_text: str):
    """Pack a '?' label onto parent_frame and attach a hover tooltip to it."""
...
ctk.CTkLabel(_wait_frame, text="Drone Menu", font=("Montserrat ExtraBold", 16)).pack(pady=(24, 8))
...
def _poll_for_game():
    try:
        ...
        return
    # Game found — swap in the main UI
...
# Auto-Click Section
# Each binding maps a key/button to a click interval (ms).
# Pressing the key toggles the auto-clicker for that binding on or off.
...
class AutoClickSection:
    ...
    # Recording — same pattern as BindingSection
    ...
    # Toggle on key press — dispatched to main thread from pynput callback
...
def _build_float_setter_row(parent, label_text: str, placeholder: str,
    ...
    # Header line: label + optional info icon + optional live current-value display
    header = ctk.CTkFrame(parent, fg_color="transparent")
...
_on_reconnect_callbacks = []  # reset functions called each time the game reconnects
...
def _build_main_ui():
    ...
    # Layout — auto-clicker first, then pitch/yaw hotkeys, then speed controls
    ...
    pitch_section = BindingSection(
        frame, "Pitch Hotkeys", set_current_pitch,
    ...
    yaw_section = BindingSection(
        frame, "Yaw Hotkeys", set_current_yaw,
    ...
    _build_float_setter_row(frame, "Deceleration",   "Default: 1",  set_deceleration,
    ...
    # Start pynput listeners now that the sections exist
    if PYNPUT_OK:
    ...
    # Begin health monitoring — checks every 3 seconds
...
# Runs every 3 seconds after the game is first attached.
# Detects if the game closes and automatically reconnects when it restarts.

def _connection_check():
    global _game_connected
    try:
        _read_ptr(gameassembly_base + FPV_BASE_OFFSET)
        app.after(3000, _connection_check)
    except Exception:
        _game_connected = False
